...
@section('content')
    <section class="container-fluid px-0">
        ...
        <section class="main-body-container">
            <section>
                <h3 class="mt-2">Edit Delivery</h3>
            </section>
            ...
            <section>
                <form action="{{ route('admin.market.delivery.update', $delivery) }}" method="post">
                    @csrf
                    @method('put')
                    <section class="row">
                        ...
                        <section class="col-12 col-md-6 my-3">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control form-control-sm" name="delivery_cost" id="delivery_cost" placeholder="optional"
                                    value="{{ old('delivery_cost', $delivery->delivery_cost) }}">
                            </div>
                            @error('delivery_cost')
                                <div class="text-danger" style="margin-top: 9px; font-size: 12px; font-weight: 400;">
                                    <strong>{{ $message }}</strong>
                        ...
                        <section class="col-12 my-3">
                            <div class="form-group">
                                ...
                                <select name="status" class="form-control form-control-sm" id="status">
                                    <option value="0" @if (old('status', $delivery->status) == 0) selected @endif>inactive
                                    </option>
                                    <option value="1" @if (old('status', $delivery->status) == 1) selected @endif>active
                                    </option>
                                </select>
                            </div>
                            @error('status')
