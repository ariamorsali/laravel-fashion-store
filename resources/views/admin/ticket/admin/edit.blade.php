...
<section class="container-fluid px-0">
    <nav style="background-color: #eee; height: 2.25rem" class="my-4 rounded ps-2" aria-label="breadcrumb">
        <ol class="breadcrumb p-1 ">
            ...
            <li class="breadcrumb-item active">Edit Admin Ticket</li>
        </ol>
    </nav>
    <section class="main-body-container">
        <section>
            <h3 class="mt-2">Edit Admin Ticket</h3>
        </section>
        ...
        <section>
            <form action="{{ route('admin.ticket.admin.update', $adminTicket) }}" method="post">
                @csrf
                @method('put')
                <section class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control form-control-sm" id="admin_id" name="admin_id">
                                ...
                                <option value="{{ $admin->id }}"
                                    @if ($admin->id == old('admin_id', $adminTicket->admin_id)) selected @endif>
                                    {{ $admin->fullName }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control form-control-sm" id="category_id" name="category_id">
                                ...
                                <option value="{{ $category->id }}"
                                    @if ($category->id == old('category_id', $adminTicket->category_id)) selected @endif>
                                    {{ $category->name }}
                                </option>
                                @endforeach
                            </select>
